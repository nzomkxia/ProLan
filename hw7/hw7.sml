...
fun eval_prog (e,env) =
	...
	(case List.find (fn (s2,v) => s=s2) env of
      ...
      | Shift (delx, dely, e) =>
        ...
        | LineSegment (x1, y1, x2, y2) => LineSegment (x1+delx, y1+dely,
                                                       x2+delx, y2+dely)
        | _ => eval_prog( Shift(delx, dely, eval_prog(e, env)), env)
...
fun preprocess_prog e =
        ...
        if real_close_point (x1,y1) (x2,y2) then Point (x1, y1)
            ...
            let
                fun compare (x1, y1, x2, y2) =
                    if real_close (x1, x2) then y1 < y2
                    else x1 < x2
            in
                if compare(x1, y1, x2, y2) then e
    ...
    | Shift ( delx, dely, e) => Shift ( delx, dely, preprocess_prog e)
    | _ => e
