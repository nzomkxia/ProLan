...
public
	# the following methods are added by Andriy, 
	# so that there is no need to add such method in subclasses of GeometryValue
	def eval_prog env 
...
class LineSegment < GeometryValue
	...
	# whether (x1, y1) should be to the left of (x2, y2)
	# assuming that (x1, y1) != (x2, y2)
	def is_valid
		if real_close(x1, x2)
			y1 < y2
		else
...
class Shift < GeometryExpression
	...
	def eval_prog env
		(@e.eval_prog env).shift(@dx, @dy).eval_prog env
	end
end
